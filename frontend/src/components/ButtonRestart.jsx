...
export default function ButtonRestart() {
    ...
    {
        try{            
            const response = await fetch("https://api:8450/save",
                {   method: "DELETE",
                    credentials:"include"
                 })
            if (response.status===200){
        ...
        try{            
            const response = await fetch(`https://api:8450/story/?parent_id=`,{method:"post", body: JSON.stringify(textPayload), credentials:"include", headers: {"Content-Type": "application/json"}})
            if (response.status===200){
