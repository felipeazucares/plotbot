  ...
  const showSpinner = () => {
    ...
    const tryGetText = async (parent_id,promptText) => {
     ...
     const payload={
      ...
      try{            
        const response = await fetch(`http://localhost:8450/story/?parent_id=${parent_id}`,{method:"post", body: JSON.stringify(textPayload), credentials:"include", headers: {"Content-Type": "application/json"}})
        if (response.status===200 && response.statusText==="OK"){
