...
export default function OrgChartTree() {
  ...
  const showSpinner = () => {
    ...
    {
        try{            
            const response = await fetch("https://api:8450/story",
                {
                    credentials:"include"
                 })
    ...
    const tryGetText = async (parent_id,promptText) => {
     ...
     const payload={
      ...
      try{            
        ...
        console.log("------------------------------------------------");
        const response = await fetch("https://api:8450/text",{method:"post", body: JSON.stringify(payload), credentials:"include", headers: {"Content-Type": "application/json"}})
        if (response.status===200){
      ...
      try{            
        const response = await fetch(`https://api:8450/story/?parent_id=${parent_id}`,{method:"post", body: JSON.stringify(textPayload), credentials:"include", headers: {"Content-Type": "application/json"}})
        if (response.status===200){
